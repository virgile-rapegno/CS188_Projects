...
def sentence3() -> Expr:
    ...
    res.append(~(PacmanAlive_0 & PacmanBorn_0))
    res.append(PacmanBorn_0)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    return conjoin([disjoin([~x for x in y]) for y in combinations])
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        GoalAssertion = conjoin(
            [~PropSymbolExpr(food_str, x, y, time=t) for x, y in food]
        )
        model = findModel(conjoin(KB) & GoalAssertion)
        print(model)
        if model != False:
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            conjoinKB = conjoin(KB)
            pacmanAt_x_y = PropSymbolExpr(pacman_str, x, y, time=t)
            isHere = entails(conjoinKB, pacmanAt_x_y)
            isNotHere = entails(conjoinKB, ~pacmanAt_x_y)

            if findModel(conjoinKB & pacmanAt_x_y):
                possible_locations.append((x, y))
            if isHere and isNotHere:
                assert True
            elif isHere:
                KB.append(pacmanAt_x_y)
            elif isNotHere:
                KB.append(~pacmanAt_x_y)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in all_coords:
            conjoinKB = conjoin(KB)
            wallAt_x_y = PropSymbolExpr(wall_str, x, y)

            isWall = entails(conjoinKB, wallAt_x_y)
            isNotWall = entails(conjoinKB, ~wallAt_x_y)

            if isWall and isNotWall:
                known_map[x][y] = -1
            elif isWall:
                KB.append(wallAt_x_y)
                known_map[x][y] = 1
            elif isNotWall:
                KB.append(~wallAt_x_y)
                known_map[x][y] = 0
